...
import {getComicByPageSize, searchComics} from "@/api/comicApi.js";
...
const searchComic = async () => {
  try {
    ...
    const response = await searchComics(title);
    console.log(response)
    comics.value = response.data; // 例如，后端返回的数据包含一个名为 comics 的数组
  } catch (error) {
